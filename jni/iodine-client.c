...
JNIEXPORT jint JNICALL Java_org_xapek_andiodine_IodineClient_connect(
	...
	const char *__p_request_encoding = (*env)->GetStringUTFChars(env, j_request_encoding, NULL);
	__android_log_print(ANDROID_LOG_ERROR, "iodine", "Request encoding from vm: %s", __p_request_encoding);
	char *p_request_encoding = strdup(__p_request_encoding);
	client_set_downenc(p_request_encoding);
	free(p_request_encoding);
	(*env)->ReleaseStringUTFChars(env, j_request_encoding, __p_request_encoding);
